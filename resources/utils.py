...
def update_cfg(cfg, params):
    if "rggb" in params["model_type"]:
        model_path = model_paths_rggb[params["yolo_type"]]
    else:
        model_path = model_paths[params["yolo_type"]]

    # if params['pretrained']:
    #     cfg.load_from = model_path
    #     cfg.resume = True
    #     max_epochs = params['epochs'] + start_epoch[params['yolo_type']]
    # else:
    # max_epochs = params['epochs']

    if params["dataset_type"] == "rgb":
        input_channels = 3
        mean = [0, 0, 0]
        std = [1, 1, 1]
        bgr_to_rgb = False
    elif params["dataset_type"] == "rggb":
        input_channels = 4
        mean = [0.0, 0.0, 0.0, 0.0]
        std = [1] * 4
        bgr_to_rgb = False
    elif params["dataset_type"] == "rggb-maxnorm":
        input_channels = 4
        mean = [0.0, 0.0, 0.0, 0.0]
        std = [params["rggb_max_train"]] * 4
        bgr_to_rgb = False
    elif params["dataset_type"] == "rggb-norm":
        input_channels = 4

        mean = mean_cocoraw_pascal
        std = std_cocoraw_pascal
    ...
    cfg.model["data_preprocessor"]["bgr_to_rgb"] = bgr_to_rgb
    cfg.model["backbone"]["input_channels"] = input_channels
    ...
    if params["backbone"] == "res":
        cfg.model["backbone"]["type"] = "PPYOLOECSPResNet"
        del cfg.model["backbone"]["last_stage_out_channels"]
    elif params["backbone"] == "dark":
        if params["preproc"] != None:
            cfg.model["backbone"]["type"] = "YOLOv8CSPDarknetPreproc"
        else:
    ...
    cfg.train_dataloader["dataset"]["data_root"] = params["data_root_train"]
    cfg.train_dataloader["dataset"]["data_prefix"]["img"] = params["train_data_prefix"]
    cfg.train_dataloader["dataset"]["ann_file"] = params["train_ann_file"]

...
for m, metric_name in enumerate(metric_names):
    for y, yolo_type in enumerate(yolos):
        ...
        axs[0,m].plot(x, pascal['rggb'], color=colors[y], marker='s', label='YOLOv8'+yolo_type)
        ...
        axs[1,m].plot(x, nod['rggb'], color=colors[y], marker='s', label='YOLOv8'+yolo_type)
        if np.sum(nod['rgb']) > 0:
